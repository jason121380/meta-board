...
export const api = {
  ...
  optimization: {
    ...
    /** Render the on-screen advice into a server-generated PDF
     *  (weasyprint) and trigger a browser download. No quota cost
     *  — the advice was already paid for when the user clicked
     *  Generate. Returns once the file is saved (or throws on
     *  network / 5xx). */
    exportPdf: async (input: {
      ...
      const resp = await fetch("/api/optimization/export-pdf", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          date_label: input.dateLabel,
          account_names: input.accountNames,
      ...
      // Server sends RFC 5987 `filename*=UTF-8''...` — decode if
      // present, otherwise fall back to a sensible default.
      let filename = "AI-staff-report.pdf";
      ...
      // Revoke after the click handler is done — Safari needs the
      // URL still alive at click time.
