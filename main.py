...
class ExportPdfAgentEntry(BaseModel):
    agent_id: str
    name_zh: str
    name_en: str
    emoji: str
...
class ExportPdfRequest(BaseModel):
    """Frontend posts the already-generated agent advice + the
    metadata we need to render a stand-alone PDF report. We don't
    re-run Gemini here — that would re-charge the user's quota for
    a download click. Frontend ships exactly what's on screen."""

    date_label: str
    account_names: List[str]
    generated_at: str  # ISO 8601, frontend's local time
    agents: List[ExportPdfAgentEntry]


def _render_pdf_html(req: ExportPdfRequest) -> str:
    """Stand-alone HTML the PDF renderer consumes. Intentionally
    different from the screen layout — this is a multi-page report
    (cover / TOC-ish summary / one section per agent) optimised for
    A4 paper and PDF reading, not a screenshot of the dashboard.

    Inline-styled because weasyprint doesn't share a CSS pipeline
    with the React app, and the report is short enough that a single
    template string is more maintainable than a Jinja file."""
    ...
    for entry in req.agents:
        body_html = md.render(entry.advice_md or "_(此位專家未產生分析)_")
        sections_html.append(
            f"""
            <section class="agent-section">
              <header class="agent-header" style="border-left-color: {h(entry.color)};">
                <div class="agent-emoji" style="background: {h(entry.color)}1f; color: {h(entry.color)};">
                  {h(entry.emoji)}
                </div>
                <div class="agent-title">
                  <h2>{h(entry.name_zh)}</h2>
                  <div class="agent-en">{h(entry.name_en)}</div>
                </div>
              </header>
    ...
    # Build cover-page metadata block. The accounts list can be
    # long; cap it visually with CSS line-clamp via a wrapping div.
    accounts_str = "、".join(req.account_names) if req.account_names else "(全部)"
    return f"""
<!doctype html>
<html lang="zh-TW">
<head>
<meta charset="utf-8" />
<title>AI 幕僚分析報告 — {h(req.date_label)}</title>
<style>
  @page {{
    size: A4;
    margin: 18mm 16mm;
    ...
    @bottom-left {{
      content: "AI 幕僚分析 · LURE Meta Platform";
      font-size: 9pt;
      color: #9ca3af;
    }}
  }}
  * {{ box-sizing: border-box; }}
  body {{
    font-family: "Noto Sans CJK TC", "Noto Sans TC", "Noto Sans CJK", sans-serif;
    color: #111827;
    font-size: 11pt;
    line-height: 1.65;
    margin: 0;
  ...
  .cover {{
    ...
    min-height: 70vh;
    padding-top: 30mm;
    border-bottom: 1pt solid #ffe8d9;
    margin-bottom: 18pt;
  }}
  .cover h1 {{
    font-size: 28pt;
  ...
  .cover .meta strong {{ color: #111827; font-weight: 600; }}
  /* Per-agent section */
  .agent-section {{
  ...
  .agent-section:last-child {{ break-after: auto; }}
  .agent-header {{
    display: flex;
    align-items: center;
    gap: 12pt;
    border-left: 4pt solid #ff6b2c;
    padding-left: 12pt;
    margin-bottom: 10pt;
  }}
  .agent-emoji {{
  ...
  /* Action-plan callout (mirrors the screen UI) */
  .agent-body h2:contains("優先"),
  .agent-body h2:contains("Priority"),
  .agent-body h2:contains("待辦") {{
    /* :contains is non-standard; the JS-side inline pass below
       converts these into a wrapped callout div instead. */
  }}
  .agent-body h3 {{
    font-size: 11pt;
    font-weight: 600;
  ...
  .agent-body li {{ margin-bottom: 3pt; }}
  .agent-body p {{ margin: 0 0 6pt; }}
  .agent-body strong {{ color: #111827; font-weight: 700; }}
  .agent-body code {{
  ...
  .callout ul, .callout ol {{ margin-top: 4pt; }}
  .callout li::marker {{ color: #ff6b2c; }}
</style>
</head>
<body>
  <div class="cover">
    <h1>AI 幕僚分析報告</h1>
    <p class="subtitle">{len(req.agents)} 位專家 · 跨帳號完整診斷</p>
    <div class="meta">
      <div><strong>資料區間</strong>:{h(req.date_label)}</div>
      <div><strong>分析帳戶</strong>:{h(accounts_str)}</div>
      <div><strong>產生時間</strong>:{h(req.generated_at)}</div>
    </div>
  </div>
  {''.join(sections_html)}
</body>
...
async def export_optimization_pdf(req: ExportPdfRequest):
    ...
    stand-alone PDF and stream the bytes back as a file download.
    Doesn't touch the agent_advice quota — that was already spent
    when the user clicked Generate; downloading the same advice
    again is free.

    The PDF layout is intentionally NOT a screenshot of the screen
    — it's a designed multi-page A4 report (cover page + one
    section per expert) so it works as a hand-off document for
    clients / internal review."""
    if not req.agents:
        raise HTTPException(status_code=400, detail="No agent advice to export")
    try:
        from weasyprint import HTML
    except ImportError as exc:
        raise HTTPException(
            status_code=503,
            detail=f"PDF 匯出未安裝(weasyprint missing): {exc}",
        )

    html_str = _render_pdf_html(req)
    # Inline pass: wrap any "優先處理" h2 + the next sibling block
    # in a `.callout` div before handing to weasyprint. We do this
    # via a simple regex so the rest of the pipeline stays HTML/CSS.
    import re

    def _wrap_callout(match: re.Match) -> str:
        h2 = match.group(0)
    ...
    filename = f"AI幕僚分析_{_dt.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    return Response(
        content=pdf_bytes,
        media_type="application/pdf",
        headers={
            # RFC 5987 filename* encoding so non-ASCII (Chinese) names
            # survive the round-trip. Plain `filename=` falls back to
            # an ASCII placeholder for older clients.
            "Content-Disposition": (
                f'attachment; filename="ai-staff-report.pdf"; '
